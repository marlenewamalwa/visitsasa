...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user && password_verify($password, $user['password'])) {
        // Store both ID and Name for header display
...
.login-container{background:white;border-radius:20px;box-shadow:0 20px 60px rgba(0,0,0,0.3);
     max-width:450px;width:100%;overflow:hidden;animation:slideUp 0.5s ease;}
@keyframes slideUp{from{opacity:0;transform:translateY(30px);}to{opacity:1;transform:translateY(0);}}
.login-header{background:linear-gradient(135deg,#0F445F 0%,#1a5c7a 100%);
...
.login-btn{width:100%;padding:16px;background:linear-gradient(135deg,#0F445F 0%,#1a5c7a 100%);
