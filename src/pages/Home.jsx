...
// ─────────────────────────────────────────────────────────────────────────────
// destinationsList comes from App.jsx (fetched once at app startup)
// Home only fetches the 3 preview cards itself
// ─────────────────────────────────────────────────────────────────────────────
export default function Home({ destinationsList = [] }) {
  const [wizardOpen,   setWizardOpen]   = useState(false);
  const [wizardDests,  setWizardDests]  = useState([]);
  const [activeSlide,  setActiveSlide]  = useState(0);
  const [animating,    setAnimating]    = useState(false);
  const [destinations, setDestinations] = useState([]);   // only 3 cards
  ...
  // ── Fetch 3 destination cards for homepage display ──
  // Note: we do NOT fetch the full list here — that comes from App.jsx via props
  useEffect(() => {
    supabase
      .from("destinations")
      .select("*")
  ...
  // Opens the wizard — optionally pre-fills a destination
  ...
  return (
    <div style={styles.page}>
      ...
      {/* ── HERO ── */}
      <section style={styles.hero} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        {HERO_SLIDES.map((s, i) => (
        ...
        <button onClick={() => changeSlide("prev")} style={{ ...styles.arrowBtn, left: 24 }} className="arrow-btn" aria-label="Previous slide">‹</button>
        <button onClick={() => changeSlide("next")} style={{ ...styles.arrowBtn, right: 24 }} className="arrow-btn" aria-label="Next slide">›</button>
      ...
      <section style={styles.editorial} className="editorial">
        <div style={styles.editorialLeft} className="editorial-left">
          ...
          <div style={styles.editorialStats} className="editorial-stats">
            {[
          ...
          <button onClick={() => openWizard()} style={styles.ctaPrimaryDark} className="btn-primary-dark btn-full-mobile">
            Build My Trip
          </button>
        </div>
      ...
      <section style={styles.destSection} className="dest-section">
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
              ...
              {destinations.map((d) => (
                <div key={d.id} style={styles.destCard} className="dest-card">
                  ...
                  <div style={styles.destInfo}>
                    ...
                    <h3 style={styles.destName} className="dest-name">{d.name}</h3>
                    <p style={styles.destDesc}>
                      {d.description ? d.description.slice(0, 100) + (d.description.length > 100 ? "…" : "") : "Discover this remarkable destination."}
                    </p>
                    <div style={styles.destActions}>
                      <Link to="/destinations" style={styles.destLearnBtn} className="dest-learn-btn">Learn more</Link>
                      <button style={styles.destCta} className="dest-cta-btn" onClick={() => openWizard([d.name])}>
                        Add to My Trip +
                      </button>
                    </div>
                  </div>
          ...
          <div style={styles.destFooter}>
            <Link to="/destinations" style={styles.seeAllBtn} className="see-all-btn">See All Destinations →</Link>
          </div>
        </div>
      </section>

      {/* ── FEATURED ACTIVITIES ── */}
      <section style={styles.actsSection} className="acts-section">
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={styles.sectionHeader} className="section-header">
          ...
          {actsLoading ? (
            <div style={styles.actsGrid} className="acts-grid">
              ...
              {activities.map((a, i) => (
                <div key={a.id} style={{ ...styles.actCard, animationDelay: `${i * 80}ms` }} className="act-card">
                  <div style={styles.actImgWrap}>
                    {a.image_url
                      ? <img src={a.image_url} alt={a.name} style={styles.actImg} className="act-img" />
                      : <div style={styles.actImgFallback} className="act-img-fallback" />
                    }
                    <div style={styles.actImgOverlay} />
                    <span style={styles.actCategoryBadge}>{a.category}</span>
                    {a.difficulty && (
                      <span style={{ ...styles.actDiffBadge, color: DIFF_COLOR[a.difficulty] || "#888" }}>{a.difficulty}</span>
                    )}
                  </div>
                  <div style={styles.actBody}>
                    <h3 style={styles.actName}>{a.name}</h3>
                    {a.description && <p style={styles.actDesc}>{a.description.slice(0, 80)}{a.description.length > 80 ? "…" : ""}</p>}
      ...
      <section style={styles.ctaBanner}>
        ...
        <div style={styles.ctaBannerContent} className="cta-banner-content">
          ...
          <button onClick={() => openWizard()} style={styles.ctaPrimary} className="btn-primary btn-full-mobile">
            Start Building Your Trip
          </button>
        </div>
      </section>
      ...
      {/* ── TRIP WIZARD ──
          destinationsList comes from App.jsx (already loaded when app started)
          so Step 0 in the wizard is instant — no loading spinner */}
      {wizardOpen && (
        <TripWizard
...
const styles = {
  ...
  heroSub: { fontSize: 15, color: "rgba(255,255,255,0.72)", lineHeight: 1.75, fontFamily: "'Helvetica Neue', sans-serif", fontWeight: 300, margin: "0 auto 36px", maxWidth: 440 },
